frappe.provide("frappe.av_patient_history")
frappe.pages['av_patient_history'].on_page_load = function(wrapper) {
	...
	let pid = '';
	page.main.html(frappe.render_template('av_patient_history', {}));
...
let setup_filters = function (patient, me) {
	...
	frappe.xcall(
		"hms_tz.hms_tz.page.av_patient_history.av_patient_history.get_patient_history_doctypes"
	).then(document_types => {
...
let get_documents = function (patient, me, document_types = "", selected_date_range = "") {
	...
	frappe.call({
		"method": "hms_tz.hms_tz.page.av_patient_history.av_patient_history.get_feed",
		args: filters,
		callback: function (r) {
			let data = r.message;
			if (data.length) {
...
frappe.pages['av_patient_history'].on_page_show = function (wrapper) {

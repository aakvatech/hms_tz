		...
		if item.reference_doctype == "Therapy Plan":
			...
			if doc.docstatus < 2:
				try:
					...
					frappe.throw("There was an error while cancelling the Item: {0} of ReferenceDoctype: {1}, ReferenceName: {2},<br>\
						Check error log for review".format(
							frappe.boled(item.item_name),
...
def return_or_cancel_drug_item(self):
	...
	unique_draft_delivery_notes = get_unique_draft_delivery_notes(self)
	if unique_draft_delivery_notes:
		for draft_delivery_note in unique_draft_delivery_notes:
			update_drug_description_for_draft_delivery_note(self, draft_delivery_note.delivery_note_no)

	unique_submitted_delivery_notes = get_unique_submitted_delivery_notes(self)
	if unique_submitted_delivery_notes:
		for dn in unique_submitted_delivery_notes:
			try:
				...
				frappe.log_error(frappe.get_traceback(), str("Error in creating return delivery note for {0}".format(dn.delivery_note_no)))
				frappe.throw("Error in creating return delivery note against delivery note: {0}".format(frappe.bold(dn.delivery_note_no)))
...
def update_drug_description_for_draft_delivery_note(self, delivey_note):
	try:
		...
		frappe.log_error(frappe.get_traceback(), str("Apply workflow error for Delivery Note {0}".format(frappe.bold(delivey_note))))
		frappe.throw(str("Apply workflow error, for delivery note: {0}, check error log for more details".format(frappe.bold(delivey_note))))
...
def update_drug_prescription_for_submitted_delivery_note(item):
	...
	frappe.db.set_value("Drug Prescription", item.child_name, {
...
def transition_workflow_states(source_doc, target_doc):
	try:
		...
		frappe.log_error(frappe.get_traceback(), str("Apply workflow error for Delivery Note {0}".format(frappe.bold(target_doc.name))))
		frappe.throw(str("Apply workflow error, for delivery note: {0}, check error log for more details".format(frappe.bold(target_doc.name))))
...
def get_unique_draft_delivery_notes(self):
...
def get_unique_submitted_delivery_notes(self):
	return frappe.db.sql("""SELECT DISTINCT(md.delivery_note_no)
		FROM `tabMedication Return` md
		INNER JOIN `tabLRPMT Returns` lrpmt ON lrpmt.name = md.parent
...
def get_drugs(patient, appointment, company):
	...
	for drug in drugs:
		item_code = frappe.get_value("Medication", drug.drug_code, "item")
		drug.update({
			"drug_code": item_code,
		})

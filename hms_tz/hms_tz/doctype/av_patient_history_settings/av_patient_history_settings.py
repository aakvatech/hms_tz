...
class AVPatientHistorySettings(Document):
	def validate(self):
		self.validate_submitable_doctypes()
		self.validate_date_fieldnames()
	
	def validate_submitable_doctypes(self):
		for entry in self.av_custom_doctypes:
...
def get_doctypes_for_patient_history():
	document_types = []
...
def validate_medical_record_required(doc):
	if frappe.flags.in_patch or frappe.in_install or frappe.flags.in_setup_wizard \
		or get_module(doc) != "Healthcare":
		return False
	
	if doc.doctype not in get_doctypes_for_patient_history():
		return False
...
def get_patient_history_config_dt(doctype):
	d = frappe.db.get_value("DocType", doctype, "custom")
	if d:
		return "AV Custom Document Type"
	
	else:
		return "AV Standard Document Type"
...
def create_medical_record(doc, method=None):
	...
	record.status = "Open"
	record.communication_date = doc.get(date_field)
	record.reference_doctype = doc.doctype
	record.reference_name = doc.name
	record.reference_owner = doc.owner
